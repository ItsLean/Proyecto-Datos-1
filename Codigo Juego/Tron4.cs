...
public class Tron4 : MonoBehaviour  
{
    ...
    IEnumerator ChangeDirectionRandomly()
    {
        ...
        yield return new WaitForSeconds(1f); // Mantener la dirección durante 1 segundos

        // Iniciar moviéndose hacia abajo
        moveDirection = Vector2.down;
        transform.rotation = Quaternion.Euler(0, 0, 270); // Ajustar rotación hacia abajo
        yield return new WaitForSeconds(1f); // Mantener la dirección inicial por 1 segundos
    ...
    IEnumerator Spawn()
    {
        while (true)
        {
            ...
            spawned.GetComponent<Tron4>().enabled = false;
